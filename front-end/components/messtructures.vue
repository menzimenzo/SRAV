  ...
  <div>
    ...
    <div>
      <b-card class="mb-3">
        <b-card-body>
          <editable
            :columns="headers"
            :data="this.listestructures.filter(str => str.sus_id == 1)"
            :removable="false"
            :creable="false"
            :editable="false"
            :noDataLabel="''"
          ...
          <b-card v-if="listestructures.filter(str => str.sus_id==1).length == 3" align="center">
          ...
          <b-btn v-if="!AjouterStructure && listestructures.filter(str => str.sus_id==1).length < 3"
                 @click="addStruct(null)" class="btn btn-primary btn-lg btn-block">
            ...
            >
              ...
              <div v-if="mastructure.tco_id == 3">
                ...
                <b-form-group v-if="boolEpci == true">
                  EPCI correspondant : <b>{{ listepci[0].com_codeinsee }} {{ listepci[0].epci_libelle }}</b>
                </b-form-group
                >
                <b-form-group v-if="boolEpci == false">
                  Aucun EPCI correspondant
                </b-form-group
                >
                <div>
                  <!--
    ...
    <div>
      <b-card class="mb-3">
        <editable
          :columns="headers"
          :data="this.listestructures.filter(str => str.sus_id == 0 || str.sus_id == 2)"
          :removable="false"
          :creable="false"
          :editable="false"
          :noDataLabel="''"
          ...
          <template slot-scope="props" slot="actions" v-if="props.data.sus_id==0">
            ...
            <b-tooltip target="boutonActive" triggers="hover">
...
export default {
  ...
  props: ["user", "statut", "submitTxt", "cancelTxt", "checkLegal"],
  methods: {
    addStruct: function () {
    ...
    activeStruct: function (strid, dcoid, strlocale, strlibelle) {
      ...
      this.mastructure.uti_structurelocale = strlocale
      this.mastructure.str_libelle = strlibelle
      if (this.listestructures.filter(str => str.sus_id==1).length === 3) {
        this.$toast.info(`Vous avez déjà 3 structures actives`)
      } else {
